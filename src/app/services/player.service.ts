...
export class PlayerService {
    ...
    getAllPlayers(): Observable<Player[]> { return this.http.get<Player[]>(this.apiUrl); }
    getPlayerById(id: number): Observable<Player> { return this.http.get<Player>(this.apiUrl + '/' + id); }

    createPlayer(command: CreatePlayerCommand): Observable<Player> {
        return this.http.post<Player>(this.apiUrl, this.toFormData(command));
    }

    updatePlayer(id: number, command: CreatePlayerCommand): Observable<Player> {
        return this.http.put<Player>(this.apiUrl + '/' + id, this.toFormData(command));
    }
    ...
    private toFormData(command: CreatePlayerCommand): FormData {
        const formData = new FormData();
        formData.append('name', command.name);
        formData.append('forname', command.forname);
        formData.append('countryCode', command.countryCode!);
